...
from minS_parallel import min_swgg
...
#                         sw2 = torch.min(W)
    ...
    for i, d in enumerate(ds):
        for l, n_projs in enumerate(projs):
            np.savetxt("./Results/Comparison_SW_projs_"+str(n_projs)+"_d"+str(d), L_sw[i, l, :, 1:])
            np.savetxt("./Results/Comparison_SWGGMC_projs_"+str(n_projs)+"_d"+str(d), L_swggmc[i, l, :, 1:])

        np.savetxt("./Results/Comparison_FC_d"+str(d), L_fc[i,:,1:])
        np.savetxt("./Results/Comparison_SW_W_d"+str(d), L_w[i,:,1:])
        np.savetxt("./Results/Comparison_SW_Sinkhorn_d"+str(d), L_s[i,:,1:])
        np.savetxt("./Results/Comparison_SWGGOptim_d"+str(d), L_swggoptim[i,:,1:])
        np.savetxt("./Results/Comparison_maxSW_d"+str(d), L_maxsw[i,:,1:])
        np.savetxt("./Results/Comparison_SRW_d"+str(d), L_srw[i,:,1:])
