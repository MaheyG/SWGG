...
def get_minSW_smooth(X, Y, lr=1e-2, num_iter=100, s=1, std=0, bar=True):
...
def upperW2_smooth(X,Y,theta,s=1,std=0):
    ...
    X_line_extend_blur = X_line_extend + 0.5 * std * torch.randn(X_line_extend.shape, device=X.device)
    Y_line_extend = Y_line_sort.repeat_interleave(s,dim=0)
    Y_line_extend_blur = Y_line_extend + 0.5 * std * torch.randn(Y_line_extend.shape, device=X.device)
