...
def swgg(X, Y, theta):
    ...
    return torch.mean(torch.sum(torch.square(X[u]-Y[v]), axis=-1), axis=0)


def min_swgg(Xs, Xt, num_projections):
    num_features = Xs.shape[1]

    projections = np.random.normal(size=(num_features, num_projections))
    projections = F.normalize(torch.from_numpy(projections), p=2, dim=0).type(Xs.dtype).to(Xs.device)
        
    return torch.min(swgg(Xs, Xt, projections))
