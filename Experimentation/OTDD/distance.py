...
from .wasserstein import efficient_pwdist_gauss, pwdist_exact, pwdist_upperbound, pwdist_means_only,pwdist_SWGG,pwdist_smooth_SWGG
...
class DatasetDistance():
    ...
    def _get_label_distances(self):
        ...
        if self.inner_ot_method == 'gaussian_approx':
            ...
            pwdist = partial(pwdist_exact,
                             ...
                             cost_function = self.feature_cost,
                             device=self.device)
        elif self.inner_ot_method=='SWGG':
            ## In this case, need to load data *before* computing label stats.

            if (self.X1 is None) or (self.X2 is None):
                self._load_datasets(maxsamples=None)  # for now, will use *all* data, to be equiv  to Gaussian
            #print(self.X1.shape)
            pwdist = partial(pwdist_SWGG,
                             symmetric=self.symmetric_tasks,
                             p = self.inner_ot_p,
                             loss = self.inner_ot_loss,
                             cost_function = self.feature_cost,
        ...
        elif self.inner_ot_method=='smooth_SWGG':
            ## In this case, need to load data *before* computing label stats.

            if (self.X1 is None) or (self.X2 is None):
                self._load_datasets(maxsamples=None)  # for now, will use *all* data, to be equiv  to Gaussian
            #print(self.X1.shape)
            pwdist = partial(pwdist_smooth_SWGG,
                             symmetric=self.symmetric_tasks,
                             p = self.inner_ot_p,
                             loss = self.inner_ot_loss,
                             cost_function = self.feature_cost,
            ...
            if return_coupling:
                with torch.no_grad():
                    ...
                    if self.coupling_method == 'geomloss':
                        ...
                        C = C.cpu()
                        π = ot.sinkhorn(torch.ones(Z1.shape[0])/Z1.shape[0], torch.ones(Z2.shape[0])/Z2.shape[0],
                                        C / C.max(), self.entreg, numItermax=500,
    ...
    def compute_coupling(self, entreg=None, gpu=None, **kwargs):
        ...
        a=torch.ones(Z1.shape[0])/Z2.shape[0]
